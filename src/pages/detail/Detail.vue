        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(item, index) in skuSaleAttrList" :key="item.id">
                <dt class="title">{{ item.saleAttrName }}</dt>
                <dd changepirce="0" @click="changecheck(item1, item.spuSaleAttrValueList)" :class="{ active: item1.isChecked == 1 }" v-for="(item1, index) in item.spuSaleAttrValueList" :key="item1.id">{{ item1.saleAttrValueName }}</dd>
              </dl>
            </div>
            <div class="cartWrap">
              <div class="controls">
              ...
              <div class="add">
                <a href="javascript:" @click='addcart($route.params.skuid,skunum)'>加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...
    skuImageList(skuInfo) {
      return this.skuInfo.skuImageList || [{}]
    },
    ...mapState({
      skuSaleAttrList(state) {
        return state.detail.Goodsinfo.spuSaleAttrList
      }
    })
  },
  watch: {
  ...
  mounted() {
    ...
    // console.log(this.skuSaleAttrList)
  },
  methods: {
    changecheck(a, b) {
      // console.log(a)
   ...
   async addcart(id,num){
    try{
      await this.$store.dispatch('AddUpdataShopCart',{id,num})
